...
async function getImageDirs() {
  ...
  const roots = [cwd, await findProjectRoot(cwd), await findProjectRoot(routeDir)].filter(Boolean).map(root => path.normalize(root));
  const candidates = [];
...
async function POST({
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const matches = String(data.imageBase64 || "").match(/^data:([^;]+);base64,(.+)$/);
      if (matches) {
        imageType = matches[1];
