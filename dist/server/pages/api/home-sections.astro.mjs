...
import { r as readHomeSections, w as writeHomeSections } from '../../chunks/home-sections_CbcuMHpY.mjs';
...
async function prepareSectionsForSave(sections = []) {
  ...
  return Promise.all(sections.map(async section => {
    ...
    if (Array.isArray(nextSection.sliderImages)) {
      nextSection.sliderImages = await Promise.all(nextSection.sliderImages.map(async image => {
        const nextImage = {
          ...image
        };
