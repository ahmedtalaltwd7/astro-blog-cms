...
import { r as readHeroConfig, w as writeHeroConfig, n as normalizeHeroConfig } from '../../chunks/hero-config_BMBZyFvS.mjs';
...
async function pruneHeroImages(keepImageUrl = "") {
  ...
  for (const imageDir of imageDirs) {
    ...
    await Promise.all(entries.filter(entry => entry.isFile() && entry.name !== keepFilename).map(async entry => {
...
async function optimizeImage(buffer) {
  ...
  }).webp(HERO_WEBP_OPTIONS).toBuffer();
  return {
...
async function POST({
  ...
  try {
    ...
    const config = await writeHeroConfig(normalizeHeroConfig({
      ...current,
      ...data,
      imageUrl
    }));
