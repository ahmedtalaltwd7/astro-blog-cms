...
export const SLIDER_ORIENTATIONS = new Set(["vertical", "horizontal"]);

export const DEFAULT_HOME_SECTIONS = [];

export function getHomeSectionsPath() {
...
function normalizeSection(section, index) {
  ...
  const wavePosition = WAVE_POSITIONS.has(section?.wavePosition)
    ? section.wavePosition
    : "top";
  const sliderOrientation = SLIDER_ORIENTATIONS.has(section?.sliderOrientation)
    ? section.sliderOrientation
    : "vertical";
...
export function normalizeHomeSections(value = []) {
  ...
  return sections
    ...
    .filter((section) => section.title || section.body || section.imageUrl || section.sliderImages.length)
    .slice(0, 12);
}
